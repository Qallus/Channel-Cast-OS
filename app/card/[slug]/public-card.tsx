...
export function PublicCard({ card, publicUrl }: { card: BusinessCard; publicUrl: string }) {
  const links = (card.links ?? []).slice().sort((a, b) => a.display_order - b.display_order);
  ...
  const effectiveCard = light ? { ...card, background_color: "#f5f7ef", text_color: "#13201a", accent_color: card.accent_color } : card;

  const track = React.useCallback((eventType: EventType, linkId?: string) => {
    navigator.sendBeacon?.("/api/cards/events", new Blob([JSON.stringify({ cardId: card.id, eventType, linkId })], { type: "application/json" })) ||
      fetch("/api/cards/events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cardId: card.id, eventType, linkId }), keepalive: true }).catch(() => {});
  }, [card.id]);
  ...
  return (
    <div className="min-h-screen w-full px-4 py-8" style={{ background: bg }}>
      {showSplash && opener && <Splash content={(opener.content || {}) as Record<string, unknown>} card={card} onView={() => setShowSplash(false)} />}
      ...
      <div className="mx-auto max-w-sm">
        ...
        <CardPreview card={effectiveCard} links={links} sections={sections} publicUrl={publicUrl} onAction={handleAction} onLink={handleLink} />
      ...
      {leadOpen && <LeadModal card={card} onClose={() => setLeadOpen(false)} onSubmitted={() => track("lead_submit")} />}
    </div>
  );
}
          ...
          <>
            ...
            <div className="mt-4 space-y-2.5">
              {fields.map((f: LeadFormField) => (
                f.key === "message" ? (
                  <textarea key={f.key} placeholder={f.label + (f.required ? " *" : "")} value={values[f.key] || ""} onChange={(e) => setValues((v) => ({ ...v, [f.key]: e.target.value }))} rows={3}
                    className="w-full resize-none rounded-lg px-3 py-2 text-sm outline-none" style={{ background: "rgba(127,127,127,0.12)", color: card.text_color, border: "1px solid rgba(127,127,127,0.2)" }} />
                ) : (
                  <input key={f.key} placeholder={f.label + (f.required ? " *" : "")} type={f.key === "email" ? "email" : f.key === "phone" ? "tel" : "text"} value={values[f.key] || ""} onChange={(e) => setValues((v) => ({ ...v, [f.key]: e.target.value }))}
                    className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={{ background: "rgba(127,127,127,0.12)", color: card.text_color, border: "1px solid rgba(127,127,127,0.2)" }} />
                )
              ))}
            </div>
