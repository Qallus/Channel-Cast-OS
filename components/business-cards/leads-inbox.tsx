...
type ConvertTarget = "contact" | "lead" | "advertiser" | "client";
const TARGETS: [ConvertTarget, string][] = [
  ["contact", "Contact"], ["lead", "Lead"], ["advertiser", "Advertiser"], ["client", "Client"],
];
...
const STATUS_STYLE: Record<LeadStatus, string> = {
  new: "bg-accent text-brand-strong",
...
export function LeadsInbox({ isAdmin, scope, onChanged }: { isAdmin: boolean; scope: "mine" | "all"; onChanged?: () => void }) {
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/business-cards/leads${isAdmin && scope === "all" ? "?scope=all" : ""}`);
  ...
  async function setStatus(lead: BusinessCardLead, status: LeadStatus) {
    ...
    await fetch(`/api/admin/business-cards/leads/${lead.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status }) }).catch(() => load());
    onChanged?.();
  }
  ...
  async function remove(lead: BusinessCardLead) {
    ...
    await fetch(`/api/admin/business-cards/leads/${lead.id}`, { method: "DELETE" }).catch(() => load());
    onChanged?.();
  }
  ...
  return (
    <div className="pt-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          {(["all", ...STATUSES] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={cn("flex items-center gap-1.5 rounded-full border px-3 py-1 text-[12px] font-medium capitalize transition", filter === f ? "border-brand-strong bg-accent text-brand-strong" : "border-border text-muted-foreground hover:text-foreground")}>
              {f}<span className="rounded-full bg-muted px-1.5 text-[10px] text-muted-foreground">{counts[f] ?? 0}</span>
            </button>
          ))}
        </div>
                ...
                <tr key={lead.id} className="align-top hover:bg-muted/30">
                  <td className="px-4 py-3">
                    ...
                    <div className="mt-0.5 flex flex-col gap-0.5 text-xs text-muted-foreground">
                      {lead.email && <a href={`mailto:${lead.email}`} className="flex items-center gap-1 hover:text-brand-strong"><Mail className="h-3 w-3" />{lead.email}</a>}
                      {lead.phone && <a href={`tel:${lead.phone}`} className="flex items-center gap-1 hover:text-brand-strong"><Phone className="h-3 w-3" />{lead.phone}</a>}
                      {lead.company && <span>{lead.company}</span>}
                    </div>
                  </td>
                  <td className="max-w-[260px] px-4 py-3 text-xs text-muted-foreground">{lead.message || "—"}</td>
                  <td className="px-4 py-3 text-xs">{lead.card_display_name || lead.card_name || "—"}</td>
                  {scope === "all" && <td className="px-4 py-3 text-xs text-muted-foreground">{lead.owner_name || "—"}</td>}
                  <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{timeAgo(lead.created_at)}</td>
                  <td className="px-4 py-3">
                    <select value={lead.status} onChange={(e) => setStatus(lead, e.target.value as LeadStatus)} className={cn("rounded-full border-0 px-2 py-1 text-[11px] font-medium capitalize outline-none", STATUS_STYLE[lead.status])}>
                      {STATUSES.map((s) => <option key={s} value={s} className="bg-card text-foreground">{s}</option>)}
                    </select>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => setConvertLead(lead)} title="Save to…" className="flex items-center gap-1 rounded-md border border-border px-2 py-1 text-[11px] text-muted-foreground hover:border-brand-strong hover:text-brand-strong">
      ...
      {convertLead && <ConvertLeadModal lead={convertLead} onClose={() => setConvertLead(null)} onConverted={() => onChanged?.()} />}
    </div>
  );
}
...
function ConvertLeadModal({ lead, onClose, onConverted }: { lead: BusinessCardLead; onClose: () => void; onConverted: () => void }) {
  ...
  const sel = "h-9 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus:border-brand-strong";

  async function convert() {
    setBusy(true);
    setError("");
    try {
      const res = await fetch(`/api/admin/business-cards/leads/${lead.id}/convert`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ target, type }) });
        ...
        {result ? (
          ...
          <>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" onClick={convert} disabled={busy}>{busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <UserPlus className="h-3.5 w-3.5" />} Save</Button>
            </div>
          </>
        )}
      </div>
