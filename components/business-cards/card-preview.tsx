...
export function CardPreview({
  card, links, sections, publicUrl, onLink, onAction,
}: {
  card: PreviewCard;
  ...
  const bg = card.background_color || "#0b1408";
  const accent = card.accent_color || "#c6ff00";
  const text = card.text_color || "#f6faf0";
  ...
  const linkRow = (label: string, onClick?: () => void, key?: string) => (
    <button key={key} type="button" onClick={onClick}
      ...
      <span>{label}</span><ExternalLink className="h-4 w-4 opacity-60" />
    </button>
  );

  function renderSection(s: BusinessCardSection) {
    const wrap = (node: React.ReactNode) => (
      <div key={s.id} style={{ marginTop: s.margin_top, marginBottom: s.margin_bottom, paddingTop: s.padding_top, paddingBottom: s.padding_bottom }}>{node}</div>
    );
    ...
    switch (s.section_type) {
      ...
      case "links": {
        ...
        for (const l of links.filter((x) => x.is_visible)) rows.push(linkRow(l.label || "Link", () => onLink?.(l), l.id));
      ...
      case "lead_capture": {
        ...
        return wrap(
          <button type="button" onClick={() => onAction?.("lead")}
      ...
      case "video": {
        if (!card.intro_video_url) {
          return wrap(<div className="rounded-xl px-4 py-3 text-xs" style={{ background: surface, border: `1px solid ${border}`, color: hexAlpha(text, 0.6) }}>
            <div className="font-semibold" style={{ color: text }}>Intro video</div>Add an intro video URL to show this section.
          </div>);
        }
      ...
      case "slideshow": {
        ...
        if (!slides.length) {
          return wrap(<div className="rounded-xl px-4 py-3 text-xs" style={{ background: surface, border: `1px solid ${border}`, color: hexAlpha(text, 0.6) }}>
            <div className="font-semibold" style={{ color: text }}>Slideshow</div>Add images in the Slideshow panel to show a gallery here.
          </div>);
        }
        return wrap(
      ...
      case "steps": {
        ...
        if (!steps.length) {
          return wrap(<div className="rounded-xl px-4 py-3 text-xs" style={{ background: surface, border: `1px solid ${border}`, color: hexAlpha(text, 0.6) }}>
            <div className="font-semibold" style={{ color: text }}>Steps</div>Add steps in the Steps panel to show a "how it works" list.
          </div>);
        }
        return wrap(
  ...
  return (
    <div className="relative mx-auto w-full max-w-sm overflow-hidden rounded-[26px] p-5"
      style={{
      ...
      <div className="mt-4 text-center text-[10px]" style={{ color: hexAlpha(text, 0.4) }}>Powered by Channel Cast</div>
    </div>
  );
}
