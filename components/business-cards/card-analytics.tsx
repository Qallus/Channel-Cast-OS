...
import { ArrowLeft, Copy, Eye, Heart, MousePointerClick, Phone, Share2, Smartphone, UserPlus, QrCode } from "lucide-react";
...
export function CardAnalyticsView({ card, onClose }: { card: BusinessCard; onClose: () => void }) {
  ...
  React.useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/admin/business-cards/${card.id}/analytics?range=${range}`)
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-lg border border-border bg-card p-0.5 text-xs">
            {[7, 30, 90].map((r) => (
              <button key={r} onClick={() => setRange(r)} className={cn("rounded-md px-3 py-1.5 font-medium", range === r ? "bg-accent text-brand-strong" : "text-muted-foreground")}>{r}d</button>
            ))}
          </div>
          <a href={`${PUBLIC_BASE}/card/${card.slug}`} target="_blank" rel="noreferrer"><Button size="sm" variant="outline"><Eye className="h-3.5 w-3.5" /> Public page</Button></a>
        </div>
      </div>
                ...
                {data.topLinks.map((l) => {
                  ...
                  return (
                    <div key={l.label}>
                      ...
                      <div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-brand" style={{ width: `${(l.count / max) * 100}%` }} /></div>
                    </div>
                  );
                })}
              </div>
