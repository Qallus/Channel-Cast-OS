...
export function BusinessCardsPage() {
  ...
  const load = React.useCallback(async (which: "mine" | "all") => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/business-cards${which === "all" ? "?scope=all" : ""}`);
  ...
  async function setStatus(card: BusinessCard, status: BusinessCard["status"]) {
    await fetch(`/api/admin/business-cards/${card.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status }) });
    load(scope);
  }

  async function remove(card: BusinessCard) {
    if (!window.confirm(`Delete "${card.card_name}"? This cannot be undone.`)) return;
    await fetch(`/api/admin/business-cards/${card.id}`, { method: "DELETE" });
    load(scope);
  }

  if (analyticsCard) return <CardAnalyticsView card={analyticsCard} onClose={() => setAnalyticsCard(null)} />;
  ...
  if (editing) {
    return (
      <CardBuilder
        ...
        currentOwner={{ name: data?.userName ?? null, email: data?.userEmail ?? null }}
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description="Employee-owned digital business cards with a public QR/NFC profile, lead capture, and analytics."
        action={
          <div className="flex flex-wrap items-center gap-2">
            {isAdmin && (
              <div className="flex items-center rounded-lg border border-border bg-card p-0.5 text-xs">
                <button onClick={() => setScope("mine")} className={cn("rounded-md px-3 py-1.5 font-medium", scope === "mine" ? "bg-accent text-brand-strong" : "text-muted-foreground")}>My cards</button>
                <button onClick={() => setScope("all")} className={cn("flex items-center gap-1 rounded-md px-3 py-1.5 font-medium", scope === "all" ? "bg-accent text-brand-strong" : "text-muted-foreground")}><Users className="h-3.5 w-3.5" />All</button>
              </div>
            )}
            {view === "cards" && <Button size="sm" variant="outline" onClick={() => load(scope)} disabled={loading}><RefreshCw className={cn("h-3.5 w-3.5", loading && "animate-spin")} /> Refresh</Button>}
            {view === "cards" && <Button size="sm" onClick={() => setEditing("new")}><Plus className="h-3.5 w-3.5" /> Create card</Button>}
          </div>
        }
      />
      ...
      <div className="flex gap-1 border-b border-border">
        {(["cards", "leads"] as const).map((v) => (
          <button key={v} onClick={() => setView(v)}
            className={cn("-mb-px flex items-center gap-2 border-b-2 px-4 py-2.5 text-sm font-medium capitalize transition", view === v ? "border-brand-strong text-brand-strong" : "border-transparent text-muted-foreground hover:text-foreground")}>
            {v === "cards" ? <IdCard className="h-3.5 w-3.5" /> : <Mail className="h-3.5 w-3.5" />}
            {v === "cards" ? "Cards" : "Leads"}
            {v === "leads" && (data?.stats.newLeads ?? 0) > 0 && (
              <span className="ml-0.5 inline-flex min-w-[18px] items-center justify-center rounded-full bg-brand px-1.5 py-0.5 text-[10px] font-semibold leading-none text-brand-foreground">
      ...
      {view === "leads" ? (
        ...
        <div>
          <div className="mb-5 flex gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-4 sm:overflow-visible xl:grid-cols-8">
            {STAT_TILES.map((t) => (
              <div key={t.key} className="w-32 shrink-0 rounded-xl border border-border bg-card px-4 py-3 sm:w-auto sm:shrink">
          ...
          <div className="mb-4 flex items-center gap-2">
            {(["all", "published", "draft"] as const).map((f) => (
              <button key={f} onClick={() => setStatusFilter(f)} className={cn("rounded-full border px-3 py-1 text-[12px] font-medium transition", statusFilter === f ? "border-brand-strong bg-accent text-brand-strong" : "border-border text-muted-foreground hover:text-foreground")}>
    ...
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {showOwner && <div className="mt-1 text-[11px] text-muted-foreground">Owner: {card.owner_name || "Unassigned"}</div>}
