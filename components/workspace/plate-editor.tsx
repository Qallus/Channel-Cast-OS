  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/admin/workspace/doc-links?${params.toString()}`);
...
function RecorderDialog({ open, onOpenChange, actionToken, onInsert, onTranscript }: { open: boolean; onOpenChange: (v: boolean) => void; actionToken: string; onInsert: (url: string) => void; onTranscript: (text: string) => void }) {
  ...
  async function transcribe() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/workspace/transcribe", { method: "POST", headers: { "x-mjg-action-token": actionToken }, body: fd });
      const data = await res.json();
...
function RecordPickerDialog({ open, onOpenChange, onInsert }: { open: boolean; onOpenChange: (v: boolean) => void; onInsert: (r: any) => void }) {
  ...
  useEffect(() => {
    if (!open) return;
    const t = setTimeout(async () => {
      try { const res = await fetch(`/api/admin/workspace/records?type=${type}&q=${encodeURIComponent(q)}`); const data = await res.json(); setResults(res.ok ? data.results : []); } catch { setResults([]); }
    }, 250);
      ...
      <DialogContent>
        ...
        <div className="flex gap-2">
          <Select value={type} onValueChange={(v) => { setType(v); setResults([]); }}>
            ...
            <SelectContent><SelectItem value="plan">Plan</SelectItem><SelectItem value="client">Client</SelectItem><SelectItem value="advertiser">Advertiser</SelectItem></SelectContent>
          </Select>
...
function AiDialog({ open, onOpenChange, getText, onInsert }: { open: boolean; onOpenChange: (v: boolean) => void; getText: () => string; onInsert: (t: string) => void }) {
  ...
  async function run(opts: { action?: string; prompt?: string }) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/workspace/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const data = await res.json();
...
export function WorkspaceEditorSurface({
  ...
  const insertDocLink = (d: any) => { try { e.tf?.insertNodes?.({ type: DocLinkPlugin.key, docId: d.id, title: d.title, href: `/app/admin/workspace/${d.id}`, children: [{ text: "" }] }); } catch { /* no-op */ } };
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="max-h-[80vh] w-[560px] overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-2 p-1">
              ...
              <InsertMegaCard onSelect={insertProjectTracker} icon={FolderKanban} title="Project Tracker" accent="hsl(var(--foreground))" desc="Assign owners, statuses, deadlines & files.">
                <span className="block space-y-1">
                  <span className="grid grid-cols-4 gap-1">{[0, 1, 2, 3].map((i) => <span key={i} className="h-1.5 rounded-full" style={{ background: "hsl(var(--foreground))" }} />)}</span>
                  {[0, 1, 2].map((r) => <span key={r} className="grid grid-cols-4 gap-1">{[0, 1, 2, 3].map((i) => <span key={i} className="h-2 rounded" style={{ background: "hsl(var(--foreground) / 0.12)" }} />)}</span>)}
                </span>
              </InsertMegaCard>
