...
function useMjgUsers() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/workspace/users").then((r) => r.json()).then((d) => { if (d.ok) { USERS_CACHE = d.users; setUsers(d.users); } }).catch(() => {});
  }, []);
  return users;
}
...
function RecordPicker({ home, onPick }: { home: string; onPick: (r: any) => void }) {
  ...
  useEffect(() => {
    let ok = true;
    const t = setTimeout(() => {
      fetch(`/api/admin/workspace/records?type=${home}&q=${encodeURIComponent(q)}`).then((r) => r.json()).then((d) => { if (ok && d.ok) setRows(d.results); }).catch(() => {});
    }, 250);
                          ...
                          {r.home !== "workspace" ? (
                            <>
                              ...
                              {r.home === "plan" && r.name.trim() && !r.recordId ? (
                                <DropdownMenuItem onSelect={async (ev) => { ev.preventDefault(); const res = await fetch("/api/admin/workspace/records", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ actionToken: token, type: "plan", name: r.name.trim() }) }); const d = await res.json(); if (d.ok) setRow(r.id, { recordId: d.recordId, href: d.href }); }}>
