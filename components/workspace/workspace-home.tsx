...
export function WorkspaceHome({ mine, shared, folders, templates, hiddenTemplateIds = [], favoriteTemplateIds = [], workspaces = [], currentWorkspaceId }: { mine: WorkspaceDocListItem[]; shared: WorkspaceDocListItem[]; folders: WorkspaceFolder[]; templates: TemplateItem[]; hiddenTemplateIds?: string[]; favoriteTemplateIds?: string[]; workspaces?: Space[]; currentWorkspaceId?: string }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/admin/workspace/search?q=${encodeURIComponent(q)}`);
  ...
  async function newDocument() {
    ...
    const data = await post("/api/admin/workspace/documents", { scope, folderId: folderId === "all" ? null : folderId, workspaceId: currentWorkspaceId });
    if (data?.id) router.push(`/app/admin/workspace/${data.id}`);
  }
  async function newFromTemplate(templateId: string) {
    const data = await post("/api/admin/workspace/documents", { scope: "personal", templateId, workspaceId: currentWorkspaceId });
    if (data?.id) router.push(`/app/admin/workspace/${data.id}`);
  }
  async function createWorkspace(name: string) {
    const data = await post("/api/admin/workspace/spaces", { name });
    setNewSpaceOpen(false);
    if (data?.workspace?.id) router.push(`/app/admin/workspace?ws=${data.workspace.id}`); else router.refresh();
  }
  async function deleteTemplate(id: string) { await post("/api/admin/workspace/templates", { templateId: id }); router.refresh(); }
  async function restoreTemplate(id: string) { await post("/api/admin/workspace/templates", { templateId: id }, "DELETE"); router.refresh(); }
  async function favoriteTemplate(id: string, on: boolean) { await post("/api/admin/workspace/templates", { templateId: id, favorite: on }, "PATCH"); router.refresh(); }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b pb-3">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-56">
              ...
              {workspaces.map((w) => (
                <DropdownMenuItem key={w.id} onSelect={() => router.push(`/app/admin/workspace?ws=${w.id}`)}>
            ...
            <Card><CardContent className="divide-y p-0">
              {docs.map((d) => (
                <div key={d.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                  <Link href={`/app/admin/workspace/${d.id}`} className="flex min-w-0 items-center gap-2 hover:underline">
            ...
            <Card><CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  {docs.map((d) => (
                    <TableRow key={d.id}>
                      <TableCell className="font-medium"><Link href={`/app/admin/workspace/${d.id}`} className="hover:underline">{d.title}</Link></TableCell>
      ...
      <NewFolderDialog open={newFolderOpen} onOpenChange={setNewFolderOpen} defaultScope={tab === "shared" ? "shared" : "personal"} onCreate={async (name, scope) => { await post("/api/admin/workspace/folders", { name, scope, workspaceId: currentWorkspaceId }); setNewFolderOpen(false); router.refresh(); }} busy={busy} />
...
function SearchResults({ hits }: { hits: SearchHit[] }) {
  ...
  return (
    <Card><CardContent className="divide-y p-0">
      {hits.map((h) => (
        <Link key={h.id} href={`/app/admin/workspace/${h.id}`} className="block px-4 py-3 hover:bg-muted/50">
  ...
  return (
    <Card className="group relative transition-colors hover:border-primary/50">
      <CardContent className="p-4">
        <Link href={`/app/admin/workspace/${doc.id}`} className="block">
...
function DocActions({ doc, onAction, refresh }: { doc: WorkspaceDocListItem; onAction: any; refresh: () => void }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      <button type="button" title="Favorite" onClick={async () => { await onAction(`/api/admin/workspace/documents/${doc.id}/favorite`, { favorite: !doc.is_favorite }); refresh(); }} className={cn("rounded p-1.5 transition-colors hover:bg-accent", doc.is_favorite ? "text-amber-500" : "text-muted-foreground hover:text-foreground")}>
      ...
      <button type="button" title="Archive" onClick={async () => { await onAction(`/api/admin/workspace/documents/${doc.id}`, { status: "archived" }, "PATCH"); refresh(); }} className="rounded p-1.5 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"><Archive className="h-4 w-4" /></button>
      ...
      <Dialog open={renameOpen} onOpenChange={setRenameOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={async () => { await onAction(`/api/admin/workspace/documents/${doc.id}`, { title }, "PATCH"); setRenameOpen(false); refresh(); }} disabled={!title.trim()}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={async () => { await onAction(`/api/admin/workspace/documents/${doc.id}`, {}, "DELETE"); setConfirmDelete(false); refresh(); }}>Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
            ...
            {items.map((d) => (
              <Card key={d.id}>
                <CardContent className="p-3">
                  <Link href={`/app/admin/workspace/${d.id}`} className="block truncate text-sm font-medium hover:underline">{d.title}</Link>
              ...
              <div className="mt-0.5 space-y-0.5">
                {items.slice(0, 3).map((doc) => (
                  <div key={doc.id} className="flex items-center gap-0.5 rounded bg-primary/15 pr-0.5 hover:bg-primary/25">
                    <Link href={`/app/admin/workspace/${doc.id}`} className="block min-w-0 flex-1 truncate px-1 py-0.5 text-[11px] text-primary" title={doc.title}>{doc.title}</Link>
