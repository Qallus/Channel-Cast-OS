        ...
        {!visible.length ? <p className="py-4 text-center text-xs text-muted-foreground">No comments yet.</p> : visible.map((c) => (
          <div key={c.id} className={cn("rounded-md border p-2", c.resolved_at && "opacity-60")}>
            ...
            <div className="mt-1 flex items-center gap-2 text-[11px] text-muted-foreground">
              ...
              <button type="button" onClick={() => act(`/api/admin/workspace/comments/${c.id}`, "PATCH", { resolved: !c.resolved_at })} className="inline-flex items-center gap-1 hover:text-foreground">{c.resolved_at ? <><RotateCcw className="h-3 w-3" /> Reopen</> : <><Check className="h-3 w-3" /> Resolve</>}</button>
              <button type="button" onClick={() => act(`/api/admin/workspace/comments/${c.id}`, "DELETE")} className="inline-flex items-center gap-1 hover:text-destructive"><Trash2 className="h-3 w-3" /> Delete</button>
            </div>

            {(repliesByParent.get(c.id) ?? []).map((r) => (
              <div key={r.id} className="mt-2 border-l pl-2">
                <CommentBody comment={r} />
                <button type="button" onClick={() => act(`/api/admin/workspace/comments/${r.id}`, "DELETE")} className="mt-0.5 inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-destructive"><Trash2 className="h-3 w-3" /> Delete</button>
              </div>
            ))}
...
function Composer({ documentId, parentId, mentionable, placeholder, onDone }: { documentId: string; parentId: string | null; mentionable: MentionUser[]; placeholder: string; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setSending(true);
    try {
      await fetch(`/api/admin/workspace/documents/${documentId}/comments`, {
        method: "POST",
